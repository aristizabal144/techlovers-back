...
class ValesController extends Controller
{
    ...
    public function create(Request $request)
    {
        try {
            ...
            $vales->faltante_pago = $request->valor;
    ...
    public function changeStatus(Request $request)
    {
        try {
            ...
            $restante_pago = $request->input('restante_pago');

            $vale = Vale::findOrFail($id);

            $vale->fecha_pago = $date;
            $vale->estado = 'pagado';
            $vale->faltante_pago = 0;

            $vale->save();

            $abonos = new AbonosVales;

            // $abonosYaRealizados = DB::table('abonos_vales')->where('id_vale', $id)->sum('valor');

            $abonos->id_vale = $id;
            $abonos->estado = $request->estado;
            $abonos->fecha = $request->fecha;
            $abonos->valor = $restante_pago;
            $abonos->descripcion = 'Ultimo pago';
